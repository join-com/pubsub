...
import { logWarnWhenUndefinedInNullPreserveFields } from './preserveNullUtil'
 ...
 */
export class PublisherMixed<T = unknown> {
  ...
  public async publishMsg(data: T): Promise<void> {
    if (!this.avroSchemasProvided) {
      ...
      try {
        ...
        //in this case we are tryin3g to resend message with avro format if schema appeared
